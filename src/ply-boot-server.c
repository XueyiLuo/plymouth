...
static void
should_quit (ply_boot_server_t *server, void *connection, bool retain_splash)
{
        static unsigned int cnt = 0;
        ply_trigger_t *quit_trigger;
        ply_boot_connection_t *con = (ply_boot_connection_t *)connection;
        if (cnt < 10 ) {
                ply_trace ("if should quit......... %d ..........",cnt);
                cnt++;
                ply_event_loop_watch_for_timeout (server->loop,
                                                          0.5,
                                                          (ply_event_loop_timeout_handler_t)
                                                          should_quit,
                                                          server);
        } else {
                ply_event_loop_stop_watching_for_timeout (server->loop,
                                                                  (ply_event_loop_timeout_handler_t)
                                                                  should_quit,
                                                                  server);
...
ply_boot_connection_on_request (ply_boot_connection_t *connection)
{
        ...
        if (strcmp (command, PLY_BOOT_PROTOCOL_REQUEST_TYPE_UPDATE) == 0) {
                ...
                ply_trace("enter should_quit");
                should_quit(server, connection, retain_splash);
                ply_trace("out should_quit");
                // quit_trigger = ply_trigger_new (NULL);
